...
export default function UniversitiesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === 'nirf')    res.sort((a,b) => a.nirf - b.nirf)
    else if (sort === 'nirfm') res.sort((a,b) => (a.nirfm||999) - (b.nirfm||999))
  ...
  const Chip = ({ label, active, onClick }: { label:string; active:boolean; onClick:()=>void }) => (
    <button onClick={onClick}
      ...
      style={{
        ...
        color:      active ? 'var(--amber-bright)':'var(--ink-2)',
      ...
      <div style={{ background:'linear-gradient(160deg,#1A2F4E,#264573)', borderBottom:'none' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
          ...
          <h1 className="font-display text-3xl md:text-4xl text-white mb-3"
            ...
            {UNIVERSITIES.length}+ Online Universities
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <div className="relative flex-1">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color:'var(--ink-3)' }} />
          ...
          <button onClick={() => setShowFilters(!showFilters)}
            ...
            style={{
              ...
              color:      showFilters ? 'var(--amber-bright)':'var(--ink-2)',
              border:     showFilters ? '1.5px solid #1A2F4E':'1.5px solid #E2E8F4',
            }}>
        ...
        {/* Filter panel */}
        {showFilters && (
          <div className="card p-5 mb-5" style={{ background:'#fff' }}>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">

              <div>
                <div className="text-xs font-bold mb-2.5 uppercase tracking-wider" style={{ color:'var(--ink-3)' }}>Program</div>
              ...
              <div>
                <div className="text-xs font-bold mb-2.5 uppercase tracking-wider" style={{ color:'var(--ink-3)' }}>Region</div>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div>
                <div className="text-xs font-bold mb-2.5 uppercase tracking-wider" style={{ color:'var(--ink-3)' }}>NAAC Grade</div>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div>
                <div className="text-xs font-bold mb-2.5 uppercase tracking-wider" style={{ color:'var(--ink-3)' }}>
            ...
            {activeFilters > 0 && (
              <div className="mt-4 pt-4" style={{ borderTop:'1px solid #F1F4F9' }}>
                <button onClick={reset} className="text-xs font-semibold" style={{ color:'#DC2626' }}>
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="text-sm font-semibold" style={{ color:'var(--ink-2)' }}>
            ...
            {activeFilters > 0 && <span style={{ color:'var(--ink-3)' }}> (filtered from {UNIVERSITIES.length})</span>}
          </div>
          {activeFilters > 0 && (
            <button onClick={reset} className="text-xs font-semibold" style={{ color:'#DC2626' }}>Clear filters</button>
          )}
        </div>

        {/* Grid */}
        {filtered.length === 0 ? (
          <div className="card p-12 text-center" style={{ background:'#fff' }}>
            <div className="text-4xl mb-4">🔍</div>
            <div className="font-semibold mb-2" style={{ color:'var(--navy)' }}>No universities match your filters</div>
            <button onClick={reset} className="text-sm font-semibold" style={{ color:'var(--amber-bright)' }}>Clear all filters →</button>
          </div>
        ) : (
