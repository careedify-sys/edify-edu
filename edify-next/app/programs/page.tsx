...
import { UNIVERSITIES, PROGRAM_META, formatFee } from '@/lib/data'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {PROGRAMS.map(prog => {
            ...
            const unis = UNIVERSITIES.filter(u => u.programs.includes(prog)).sort((a, b) => a.nirf - b.nirf)
            if (unis.length === 0) return null
            const minFee = Math.min(...unis.map(u => u.feeMin))
            return (
              <Link
                key={prog}
                href={`/universities?prog=${prog}`}
