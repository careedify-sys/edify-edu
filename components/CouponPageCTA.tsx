...
import { TIER_AMOUNTS } from '@/lib/coupons'
  ...
  if (page.couponCode === 'N/A') {
    return (
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-center mb-8">
        ...
        <h3 className="text-lg font-bold text-slate-700 mb-2">No enrollment bonus needed - {page.shortName} has the lowest fee</h3>
    ...
    <>
      <div className="rounded-2xl border-2 border-amber-300 bg-gradient-to-br from-amber-50 via-orange-50 to-white p-6 sm:p-8 text-center mb-8">
        <div className="text-xs font-bold uppercase tracking-wider text-amber-700 mb-2">
          edifyedu.in enrollment bonus - {page.shortName} ({page.tier} tier)
        </div>
        ...
        <p className="text-sm text-slate-600 mb-2">
          Up to <strong>Rs {tierMax.toLocaleString('en-IN')}</strong> on Tuesday and Saturday in IST, <strong>Rs {tierBase.toLocaleString('en-IN')}</strong> on other days.
        </p>
        ...
        {revealed ? (
          <p className="text-sm text-green-700 font-bold mb-2">
            Code unlocked. Mention <strong>{page.couponCode}</strong> on your advisor call.
          </p>
        ) : (
          <p className="text-sm text-slate-600 mb-4">
            Fill a 30-second form and our advisor will confirm the active amount and apply the bonus.
          </p>
        )}
        <button
          type="button"
        ...
        >
          ...
          {revealed ? 'Speak to Advisor Now' : 'Reveal Enrollment Bonus Code'}
        </button>
        <p className="text-xs text-slate-500 mt-3">Free. No spam. Our advisor confirms the bonus with {page.shortName} admissions.</p>
      </div>
