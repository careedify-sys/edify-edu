  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      ...
      <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-sm font-bold text-slate-800">Compare Syllabus Side by Side</h2>
...
// ── University card used in mobile comparison view ───────────────────────────
function UniCard({
  u,
  idx,
  program,
    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-bold text-slate-800 leading-snug">{u.name}</h3>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            {u.nirf && (
              <span className="text-xs font-bold text-slate-700">#{u.nirf} NIRF</span>
            )}
      ...
      <div className="px-4 py-4 bg-slate-50 border-t border-slate-100 flex flex-col gap-2">
        <button
          onClick={onEnquire}
          className="w-full bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold py-2.5 rounded-xl transition-colors"
        >
          Enquire Now
        </button>
        <Link
          href={`/universities/${u.id}/${progSlug(program)}`}
          className="text-xs text-center text-amber-600 hover:text-amber-700 font-semibold"
        >
          View Full Analysis →
        </Link>
      </div>
  ...
  return (
    ...
    >
      ...
      {unis.map((u, idx) => (
        <div
          key={u.id}
          className="p-4 py-5 border-l border-slate-100 flex flex-col items-center justify-center text-center bg-white"
        >
          {fn(u, idx)}
        </div>
      ))}
      ...
      <div style={{ background: 'linear-gradient(160deg, #0d2240 0%, #1e3a5f 100%)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-8 pb-8 sm:pt-12 sm:pb-10">
          ...
          <h1 className="font-display text-2xl sm:text-4xl font-bold text-white text-center mb-3 leading-tight">
            Compare Online{' '}
            <span className="text-amber-400">{program}</span>{' '}
            Universities
          </h1>
          <p className="text-white/50 text-sm text-center max-w-xl mx-auto mb-7 leading-relaxed">
            Side-by-side analysis of UGC DEB approved {program} programs — fees, NIRF rankings
            {program === 'MBA' ? ', specialisations' : ''} and semester-wise syllabus.
          </p>

          {/* Program toggle */}
          <div className="flex justify-center mb-7">
            <div className="flex p-1 rounded-xl border border-white/10 bg-white/5 gap-1 w-full sm:w-auto">
              {(['MBA', 'MCA'] as const).map(p => (
                <button
                  key={p}
                  onClick={() => { setProgram(p); setSelectedIds(['jain-university-online', 'amity-university-online']) }}
                  className={clsx(
                    "flex-1 sm:flex-none px-6 py-2 text-sm font-semibold rounded-lg transition-all",
                    program === p
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              { icon: <GraduationCap size={13} />, label: '127+ Universities' },
              { icon: <ShieldCheck size={13} />, label: 'UGC DEB Verified' },
              { icon: <Zap size={13} />, label: '24 Hr Callback' },
            ].map(badge => (
              <div key={badge.label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/8 border border-white/10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-2.5">
            ...
            {universities.map((u) => (
              <div
                key={u.id}
                className="flex items-center gap-1.5 pl-2 pr-1 py-1 rounded-lg bg-slate-100 border border-slate-200"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {universities.length === 0 ? (
          <div className="py-24 text-center">
            ...
            <h2 className="text-lg font-bold text-slate-700 mb-2">No Universities Selected</h2>
          ...
          <div>
            ...
            {verdict && (
              <div className="mt-6 mb-6 border border-slate-200 rounded-2xl bg-white p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-slate-800">The Edify Verdict</h3>
            ...
            <div className="md:hidden space-y-4 mb-8">
              <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">University Details</h2>
            ...
            <div className="hidden md:block mb-8 border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              <div className="overflow-x-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
                ...
                {/* Sticky header */}
                <div
                  className="sticky top-[57px] z-[35] border-b border-slate-200 bg-white"
                >
                  <div
                    className="grid"
                    style={{ gridTemplateColumns: `160px repeat(${universities.length}, minmax(200px, 1fr))` }}
                  >
                        ...
                        <div className="h-10 flex items-center justify-center mb-2">
                          {getUniversityLogo(u.id) ? (
                            <img
                              ...
                              alt={u.name}
                              style={{ maxHeight: '100%', maxWidth: '130px', objectFit: 'contain' }}
                            />
                          ) : (
                ...
                <ComparisonRow
                  label="NIRF Rank"
                  icon={<BarChart2 size={12} className="text-amber-500" />}
                  unis={universities}
                  fn={(u) => u.nirf
                    ? <span className="text-lg font-bold text-slate-800">#{u.nirf}</span>
                ...
                <ComparisonRow
                  label="Avg Salary"
                  unis={universities}
                  fn={(u) => <SalaryDisplay u={u} />}
                />
                <ComparisonRow
                  label="Hiring Partners"
                  unis={universities}
                  fn={(u) => <CompaniesDisplay u={u} />}
                />
                <ComparisonRow
                ...
                >
                  ...
                  {universities.map(u => (
                    <div key={u.id} className="p-4 border-l border-slate-100 flex flex-col gap-2">
                      ...
                      <Link
                        href={`/universities/${u.id}/${progSlug(program)}`}
                        className="text-[11px] font-semibold text-center text-amber-600 hover:text-amber-700"
                      >
                        View Full Analysis →
                      </Link>
                    </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
              ...
              <div className="p-5 bg-white border border-slate-200 rounded-2xl">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-slate-800">Frequently Compared</h3>
                </div>
                <div className="space-y-2">
                  {(program === 'MCA' ? [
                    ...
                    <div
                      key={c}
                      className="flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:border-amber-300 hover:bg-amber-50 transition-all cursor-pointer group"
                    >
                      <span className="text-xs font-medium text-slate-600 group-hover:text-slate-800">{c}</span>
