...
function extractManualPostBlocks(existingBlogTS: string, sheetSlugs: Set<string>): string[] {
  // Find the BLOG_POSTS array content between [ and final ]
  const arrayStart = existingBlogTS.indexOf('export const BLOG_POSTS')
  if (arrayStart === -1) return []
  const openBracket = existingBlogTS.indexOf('[', arrayStart)
  if (openBracket === -1) return []
  // Find closing ] — last one before BLOG_CATEGORIES
  const catsIdx = existingBlogTS.indexOf('export const BLOG_CATEGORIES', openBracket)
  const closeBracket = catsIdx !== -1
    ? existingBlogTS.lastIndexOf(']', catsIdx)
    : existingBlogTS.lastIndexOf(']')
  if (closeBracket === -1) return []

  const postsRegion = existingBlogTS.slice(openBracket + 1, closeBracket)

  // Split into individual post blocks using the consistent "  {\n    slug:" pattern
  const blockBoundary = /(?:^|\n)  \{(?=\n\s+slug:)/g
  const splitPoints: number[] = []
  let bm: RegExpExecArray | null
  while ((bm = blockBoundary.exec(postsRegion)) !== null) {
    splitPoints.push(bm.index)
  }

  const manualBlocks: string[] = []
  for (let i = 0; i < splitPoints.length; i++) {
    const blockStart = splitPoints[i]
    const blockEnd = i + 1 < splitPoints.length ? splitPoints[i + 1] : postsRegion.length
    const block = postsRegion.slice(blockStart, blockEnd).trim().replace(/,\s*$/, '')

    // Extract slug from this block
    const slugMatch = block.match(/slug:\s*'([^']+)'/)
    if (!slugMatch) continue
    const slug = slugMatch[1]

    // Only preserve posts NOT managed by the Google Sheet
    if (!sheetSlugs.has(slug)) {
      manualBlocks.push('  ' + block.replace(/^\s+/, '').replace(/^  /, '  '))
    }
  }
  return manualBlocks
}
...
function generateBlogTS(
  ...
  // Build set of slugs managed by the Google Sheet
  const sheetSlugs = new Set(
    posts
      .map(p => String(p['Slug (URL)'] ?? '').trim().replace(/^\/+/, '').replace(/^blog\//, ''))
      .filter(Boolean)
  )
  ...
  const postBlocks = posts.map(p => {
    ...
    // No length cap — CMS posts can be as long as needed
...
${faqs.filter(f => f.q && f.a).map(f => `      { q: '${ts(f.q, 200)}', a: '${ts(f.a, 500)}' }`).join(',\n')}
  ...
  // Preserve manually-added posts (those not managed by the Google Sheet)
  const manualBlocks = existingBlogTS
    ? extractManualPostBlocks(existingBlogTS, sheetSlugs)
    : []

  const allBlocks = [...postBlocks, ...manualBlocks]
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const blogTS = generateBlogTS(cmsData.blogs, existingDates, existingBlogTS)
    const manualCount = (blogTS.match(/slug:\s*'/g) || []).length - cmsData.blogs.length
    addLog(`✓ blog.ts: ${cmsData.blogs.length} sheet posts + ${manualCount > 0 ? manualCount : 0} manual posts preserved`)
